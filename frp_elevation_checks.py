...
def convert_to_points(xs, pbl, workspace):
    ...
    # Output path to the new feature class
...
def drop_fields(in_fc):
    ...
    # Fields to drop
    fields_to_drop = [field for field in field_names if field not in keep_fields]

    # Drop the fields
    for drop_field in fields_to_drop:
        if drop_field.lower() != 'shape':
...
def calc_scrv(in_fc, dem):
    ...
    for water_name in water_names:
        ...
        # Create a list of lists where the elements are the stream station and DEM value
        station_list = [
